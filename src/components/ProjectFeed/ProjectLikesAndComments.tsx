...
import useLikeProject from "@/services/action/useLikeProject";
import useGetProjectLikesAndComments from "@/services/query/useGetProjectLikesAndComments";
import { Skeleton } from "../ui/skeleton";
import Link from "next/link";

export default function ProjectLikesAndComment({
  projectId,
}: {
  projectId: string;
}) {
  const { isLoading, error, data } = useGetProjectLikesAndComments(projectId);
  ...
  return (
    <section className="flex items-center justify-between gap-4 w-full">
      ...
      <Link
        ...
        className="p-5 border rounded-xl flex-1"
      >
        Write a comment...
      </Link>
