...
import usePostComment from "@/services/action/usePostComment";
import useGetPostComments from "@/services/query/useGetPostComments";
import { Button } from "../ui/button";
import { Input } from "../ui/input";
import { Skeleton } from "../ui/skeleton";

export default function PostComments({ postId }: { postId: string }) {
  const { data, isLoading, error } = useGetPostComments(postId);
        ...
        {isLoading ? (
          ...
          data.map((comment) => (
            <div
              ...
              key={comment.post_comments.postCommentId}
            >
              <Image
              ...
              <div>
                <div className="space-y-2 bg-wheat_white px-4 py-2 rounded-xl min-w-[250px] max-w-full">
                  ...
                  <div className="text-gray-700 text-sm">
                    {comment.post_comments.commentText}
                  </div>
                </div>
                <div className="mt-1 text-gray-700 ml-auto text-xs text-right">
                  {comment.post_comments.createdAt &&
                    new Date(comment.post_comments.createdAt).toDateString()}
                </div>
              </div>
            </div>
          ))
